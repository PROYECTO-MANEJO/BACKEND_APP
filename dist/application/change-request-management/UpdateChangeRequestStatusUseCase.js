...
class UpdateChangeRequestStatusUseCase {
    constructor(changeRequestRepository, workflowService) {
    ...
    async execute(data) {
        ...
        const validation = this.workflowService.validateStatusTransition(changeRequest.status, data.newStatus, data.userRole, changeRequest);
        if (!validation.isValid) {
            throw new Error(validation.reason || "Transición de estado no válida");
        }
        // Verificar permisos específicos
        this.validateUserPermissions(changeRequest, data.userId, data.userRole, data.newStatus);
        ...
        switch (data.newStatus) {
            ...
            default:
                ...
                throw new Error(`Transición a estado ${data.newStatus} no implementada específicamente`);
        }
    ...
    validateInput(data) {
        ...
        if (!validRoles.includes(data.userRole)) {
            throw new Error(`Rol inválido. Valores válidos: ${validRoles.join(", ")}`);
        }
        if (data.comments && data.comments.length > 1000) {
            throw new Error("Los comentarios no pueden exceder 1000 caracteres");
        }
    }
    validateUserPermissions(changeRequest, userId, userRole, newStatus) {
        ...
        if (newStatus === "EN_TESTING" && userRole === "DESARROLLADOR") {
            if (changeRequest.developerId !== userId) {
                throw new Error("Solo el desarrollador asignado puede enviar a testing");
            }
        }
