...
const AuthValidator_1 = require("../../domain/validators/AuthValidator");
class AuthService {
    ...
    async login(loginRequest) {
        ...
        AuthValidator_1.AuthValidator.validateLogin({
        ...
        const authData = await authRepository.findCompleteByEmail(loginRequest.email);
        ...
        const validPassword = await bcrypt.compare(loginRequest.password, authData.user.password || "");
        if (!validPassword) {
            return {
                success: false,
        ...
        let token;
        if (authData.account.role === "ADMINISTRADOR" ||
    ...
    async register(registerRequest) {
        // ✅ SRP: Delegar validación al AuthValidator
        AuthValidator_1.AuthValidator.validateRegister(registerRequest);
        // ✅ SRP: Obtener dependencias del container
        ...
        const existingAccountByEmail = await authRepository.isEmailTaken(registerRequest.email);
        ...
        const existingUserByCedula = await authRepository.isCedulaTaken(registerRequest.ced_usu);
        ...
        if (registerRequest.email.endsWith("@uta.edu.ec") &&
    ...
    async createAdmin(adminRequest) {
        // ✅ SRP: Delegar validación al AuthValidator
        AuthValidator_1.AuthValidator.validateCreateAdmin(adminRequest);
        // ✅ SRP: Obtener dependencias del container
        const userRepository = this.container.getUserRepository();
        const bcrypt = this.container.getBcrypt();
        const prisma = this.container.getPrismaClient();
        // ✅ SRP: Verificar si ya existe usuario con esta cédula
        const existingUserByCedula = await userRepository.findByCedula(adminRequest.ced_usu);
        if (existingUserByCedula) {
            return {
                success: false,
