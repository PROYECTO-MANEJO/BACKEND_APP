...
const EventValidator_1 = require("../../domain/validators/EventValidator");
const EventBusinessRules_1 = require("../../domain/services/EventBusinessRules");
class EventService {
    ...
    async createEvent(eventData) {
        ...
        EventValidator_1.EventValidator.validate(eventDataForValidation);
        // Crear entidad limpia
        const event = new Event_1.Event(eventDataForValidation);
        ...
        return new Event_1.Event(result);
    }
    /**
    ...
    async getEventById(id) {
        ...
        return eventData ? new Event_1.Event(eventData) : null;
    }
    /**
    ...
    async getAllEvents() {
        ...
        return eventsData.map((data) => new Event_1.Event(data));
    }
    /**
     * Actualizar evento
     */
    async updateEvent(id, updateData) {
        ...
        const existingEvent = new Event_1.Event(existingEventData);
        // Verificar reglas de negocio
        if (!EventBusinessRules_1.EventBusinessRules.canBeUpdated(existingEvent.toPlainObject())) {
            throw new Error("Event cannot be updated");
        }
        // Validar campos individuales si se proporcionan
        if (updateData.nom_eve !== undefined) {
            EventValidator_1.EventValidator.validateName(updateData.nom_eve);
        }
        if (updateData.des_eve !== undefined) {
            EventValidator_1.EventValidator.validateDescription(updateData.des_eve);
        }
        if (updateData.capacidad_max_eve !== undefined) {
            EventValidator_1.EventValidator.validateCapacityValue(updateData.capacidad_max_eve);
        }
        if (updateData.precio !== undefined) {
            EventValidator_1.EventValidator.validatePriceValue(updateData.precio, existingEvent.es_gratuito);
        }
        // Actualizar usando método limpio de la entidad
        existingEvent.updateData(updateData);
        // Persistir cambios
        const updatedEventData = await eventRepository.update(id, existingEvent.toPlainObject());
        if (!updatedEventData) {
            throw new Error("Failed to update event");
        }
        return new Event_1.Event(updatedEventData);
    }
    /**
     * Eliminar evento
     */
    async deleteEvent(id) {
        ...
        // Verificar reglas de negocio
        if (!EventBusinessRules_1.EventBusinessRules.canBeDeleted(existingEventData)) {
    ...
    async closeEvent(id) {
        ...
        EventBusinessRules_1.EventBusinessRules.closeEvent(existingEvent);
