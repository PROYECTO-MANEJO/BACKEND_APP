...
exports.CourseCategoryManagement = void 0;
const courses_1 = require("../../../domain/entities/courses");
class CourseCategoryManagement {
    constructor(categoryRepository, validationService, courseService, notificationService, permissionService) {
    ...
    async createCategory(input, userId) {
        try {
            ...
            const category = courses_1.CourseCategory.create(input.name, input.description || "", input.code, input.color, input.createdBy);
            // Save to repository
    ...
    async updateCategory(input, userId) {
        try {
            ...
            if (input.name !== undefined ||
                ...
                updatedCategory = updatedCategory.updateBasicInfo(input.name, input.description, input.code, input.color, input.updatedBy);
            }
            ...
            try {
                ...
                if (changes.length > 0) {
                    await this.notificationService.notifyCategoryUpdated(updatedCategory, changes, [input.updatedBy || "system"]);
                }
            }
            catch (error) {
    ...
    async deleteCategory(categoryId, userId) {
        try {
            ...
            // Send notification
            try {
                await this.notificationService.notifyCategoryDeleted(category.getName(), coursesInCategory.length, [userId || "system"]);
            }
            catch (error) {
    ...
    async getCategories(filters) {
        try {
            ...
            if (filters?.name) {
                ...
                filteredCategories = categories.filter((category) => category.getName().toLowerCase().includes(nameFilter) ||
     ...
     */
    async updateCategoryStatistics(categoryId, statistics, userId) {
        try {
            const category = await this.categoryRepository.findById(categoryId);
            if (!category) {
                return {
     ...
     */
    async incrementCourseCount(categoryId, courseStatus = "ACTIVE", userId) {
        try {
            const category = await this.categoryRepository.findById(categoryId);
            if (!category) {
                return {
     ...
     */
    buildChangesList(originalCategory, updatedCategory) {
        const changes = [];
        ...
        if (originalCategory.getParentCategoryId() !==
...
exports.CourseCategoryManagement = CourseCategoryManagement;
//# sourceMappingURL=CourseCategoryManagement.js.map