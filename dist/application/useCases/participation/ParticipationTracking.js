...
exports.ParticipationTracking = void 0;
class ParticipationTracking {
    constructor(participationRepository, enrollmentRepository, certificateService, notificationService) {
    ...
    async recordAttendance(input) {
        try {
            ...
            const updatedParticipation = participation.recordAttendance(input.sessionDate, input.present, input.checkInTime, input.checkOutTime, input.notes, input.recordedBy);
    ...
    async updateGrade(input) {
        try {
            ...
            const updatedParticipation = participation.updateGrade(input.finalGrade, input.evaluatedBy, input.comments);
            await this.participationRepository.update(updatedParticipation);
            ...
            if (updatedParticipation.isCompleted() &&
     ...
     */
    async updateAttendancePercentage(input) {
        try {
            const participation = await this.participationRepository.findById(input.participationId);
            if (!participation) {
                return {
            ...
            const updatedParticipation = participation.updateAttendancePercentage(input.attendancePercentage, input.evaluatedBy);
            await this.participationRepository.update(updatedParticipation);
            // Check if participation is now completed and eligible for certificate
            if (updatedParticipation.isCompleted() &&
                updatedParticipation.canGenerateCertificate()) {
     ...
     */
    async generateCertificate(participationId) {
        try {
            ...
            const certificateResult = await this.certificateService.generateCertificate(participation.getId(), participation.getParticipantName(), participation.getActivityName(), new Date());
            ...
            if (certificateResult.certificateUrl) {
                await this.notificationService.sendCertificateNotification(updatedParticipation, certificateResult.certificateUrl);
            }
            return {
                success: true,
     ...
     */
    async checkCertificateEligibility(participationId) {
        try {
            const participation = await this.participationRepository.findById(participationId);
            if (!participation) {
                return {
     ...
     */
    async withdrawParticipant(participationId, reason, withdrawnBy) {
        try {
            const participation = await this.participationRepository.findById(participationId);
            if (!participation) {
                return {
    ...
    async getParticipations(filters) {
        try {
            ...
            const participationSummaries = participations.map((participation) => participation.getParticipationSummary());
            return {
                success: true,
     ...
     */
    async processPendingEvaluations() {
        try {
            ...
            for (const participation of pendingParticipations) {
                try {
                    ...
                    if (participation.getAttendancePercentage() >=
                        ...
                        if (participation.getActivityType() === "EVENT") {
                            ...
                            updatedParticipation = participation.updateAttendancePercentage(participation.getAttendancePercentage(), "system");
                        }
                        else {
                            ...
                            if (grading.finalGrade !== undefined &&
                                ...
                                updatedParticipation = participation.updateGrade(grading.finalGrade, "system", "Auto-evaluated based on requirements");
                            }
                            else {
     ...
     */
    async generateEligibleCertificates() {
        try {
...
exports.ParticipationTracking = ParticipationTracking;
//# sourceMappingURL=ParticipationTracking.js.map