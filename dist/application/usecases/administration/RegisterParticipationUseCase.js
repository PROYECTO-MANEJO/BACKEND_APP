...
exports.RegisterParticipationUseCase = void 0;
const ParticipationRegistration_1 = require("../../../domain/entities/administration/ParticipationRegistration");
class RegisterParticipationUseCase {
    constructor(participationRepo, activityRepo, certificateService, notificationService) {
    ...
    async execute(request) {
        try {
            ...
            const existingParticipation = await this.participationRepo.findByUserAndActivity(request.userId, request.activityId, request.activityType);
            ...
            const participation = ParticipationRegistration_1.ParticipationRegistration.create(participationData.participantId, participationData.participantName, participationData.participantEmail, participationData.participantCedula, participationData.activityId, participationData.activityTitle, participationData.participationType, participationData.registeredBy);
            // Registrar asistencia si se proporciona
            let updatedParticipation = participation;
            if (request.attendanceData?.checkInTime) {
                updatedParticipation = participation.markAttendance(request.attendanceData.attendancePercentage || 100);
            ...
            if (certificateEligible) {
                try {
                    certificateData = await this.certificateService.generateCertificate(savedParticipation.getId(), request.activityType, {
                        participantName: request.userName,
                        activityTitle: activityInfo.title,
                        completionDate: new Date(),
                    });
                    // Actualizar participación con datos del certificado
                    const updatedParticipation = savedParticipation.generateCertificate(certificateData.certificateId);
                    await this.participationRepo.update(updatedParticipation);
                    certificateGenerated = true;
                    // Notificar generación de certificado
                    await this.notificationService.sendCertificateGeneratedNotification(request.userId, activityInfo.title, certificateData.certificateId, certificateData.downloadUrl);
                }
                catch (certError) {
            ...
            try {
                await this.notificationService.sendParticipationRegisteredNotification(request.userId, activityInfo.title, request.activityType, certificateEligible);
            }
            catch (notificationError) {
                console.error("Failed to send participation notification:", notificationError);
            }
            // Calcular próximos pasos
            const nextSteps = this.calculateNextSteps(savedParticipation.getStatus(), certificateEligible, certificateGenerated);
            return {
                success: true,
    ...
    async updateExistingParticipation(existingParticipation, request) {
        // Si ya existe, actualizar con nueva asistencia
        let updatedExistingParticipation = existingParticipation;
        if (request.attendanceData?.checkInTime) {
            updatedExistingParticipation = existingParticipation.markAttendance(request.attendanceData.attendancePercentage || 100);
            if (request.attendanceData.notes) {
        ...
        return {
            ...
            nextSteps: this.calculateNextSteps(updatedExistingParticipation.getStatus(), certificateEligible, certificateGenerated),
        };
    }
    validateRequest(request) {
        ...
        if (request.attendanceData) {
            if (request.attendanceData.attendancePercentage &&
                (request.attendanceData.attendancePercentage < 0 ||
                    request.attendanceData.attendancePercentage > 100)) {
                throw new Error("Attendance percentage must be between 0 and 100");
            }
            if (request.attendanceData.checkInTime &&
                request.attendanceData.checkOutTime &&
                request.attendanceData.checkInTime >=
                    request.attendanceData.checkOutTime) {
                throw new Error("Check-in time must be before check-out time");
            }
        }
    }
    calculateNextSteps(currentStatus, certificateEligible, certificateGenerated) {
        const steps = [];
...
exports.RegisterParticipationUseCase = RegisterParticipationUseCase;
//# sourceMappingURL=RegisterParticipationUseCase.js.map