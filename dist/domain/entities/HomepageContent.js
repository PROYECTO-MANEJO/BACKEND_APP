...
class HomepageContent {
    ...
    static create(heroTitle, heroSubtitle, heroDescription, editorId, editorName) {
        const now = new Date();
        const contentData = {
    ...
    static fromPrismaData(homepageData) {
        const contentData = {
    ...
    static mapImagesFromPrisma(homepageData) {
        const images = {};
    ...
    // Actions
    updateHeroSection(title, subtitle, description, editorId, editorName) {
        const updatedData = {
            ...this.data,
    ...
    updateOfferSection(title, subtitle, editorId, editorName) {
        const updatedData = {
            ...this.data,
    ...
    updateSection(sectionId, title, description, editorId, editorName) {
    ...
    updateFooter(text1, text2, text3, editorId, editorName) {
        const updatedData = {
            ...this.data,
    ...
    uploadImage(type, buffer, mimeType, editorId, editorName) {
    ...
    removeImage(type, editorId, editorName) {
    ...
    toggleSectionVisibility(sectionId, editorId, editorName) {
        const sectionIndex = this.data.sections.findIndex((s) => s.id === sectionId);
        if (sectionIndex === -1) {
            throw new Error(`Section ${sectionId} not found`);
        }
    ...
    getContentStatistics() {
        return {
            ...
            lastUpdateDaysAgo: Math.floor((new Date().getTime() - this.data.lastUpdateDate.getTime()) /
                (1000 * 60 * 60 * 24)),
        };
    }
    calculateCompletionPercentage() {
        ...
        totalItems += 3;
        if (this.data.heroTitle)
            completedItems++;
        if (this.data.heroSubtitle)
            completedItems++;
        if (this.data.heroDescription)
            completedItems++;
        ...
        // Footer (3 items)
        totalItems += 3;
        if (this.data.footerText1)
            completedItems++;
        if (this.data.footerText2)
            completedItems++;
        if (this.data.footerText3)
            completedItems++;
        // Images (optional, 5 items)
        totalItems += 5;
        Object.keys(this.data.images).forEach((key) => {
            if (this.data.images[key]?.buffer)
                completedItems++;
        });
