...
exports.ChangeRequestWorkflowService = void 0;
class ChangeRequestWorkflowService {
    ...
    validateStatusTransition(currentStatus, newStatus, userRole, request) {
        const rule = this.statusTransitionRules.find((rule) => rule.from === currentStatus && rule.to === newStatus);
        if (!rule) {
            return {
                isValid: false,
     ...
     */
    getValidNextStates(currentStatus, userRole) {
        return this.statusTransitionRules
            .filter((rule) => rule.from === currentStatus && rule.requiredRole.includes(userRole))
            .map((rule) => rule.to);
    }
    /**
     * Calcular la prioridad efectiva de una solicitud
     */
    calculateEffectivePriority(request) {
        ...
        const baseScore = (priorityWeights[summary.priority] || 2) *
            (urgencyWeights[summary.urgency] || 1);
     ...
     */
    recommendDeveloper(request, availableDevelopers) {
        if (availableDevelopers.length === 0) {
            return null;
        }
        // Filtrar desarrolladores disponibles
        const eligibleDevelopers = availableDevelopers.filter((dev) => dev.canTakeNewRequest());
     ...
     */
    validateTechnicalPlans(request) {
        const summary = request.getSummary();
        const missingPlans = [];
     ...
     */
    estimateResolutionTime(request) {
        const summary = request.getSummary();
        ...
        baseDays *=
            typeMultipliers[summary.type] || 1;
...
exports.ChangeRequestWorkflowService = ChangeRequestWorkflowService;
//# sourceMappingURL=ChangeRequestWorkflowService.js.map