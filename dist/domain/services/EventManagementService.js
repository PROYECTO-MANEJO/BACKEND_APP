...
class EventManagementService {
    constructor(eventRepository, categoryRepository, userRepository) {
    ...
    async createEvent(data) {
        ...
        const categoryExists = await this.categoryRepository.existsById(data.id_cat_eve);
        if (!categoryExists) {
            throw new Error("La categoría del evento no existe");
        }
        // Validar que el organizador exista y sea válido
        const organizerExists = await this.userRepository.existsByCedula(data.ced_org_eve);
        if (!organizerExists) {
        ...
        const isValidOrganizer = await this.userRepository.isOrganizer(data.ced_org_eve);
        if (!isValidOrganizer) {
    ...
    async getEventsPaginated(page = 1, limit = 10, filters) {
        if (page < 1)
            page = 1;
        if (limit < 1)
            limit = 10;
        if (limit > 100)
            limit = 100; // Límite máximo de seguridad
    ...
    async getUserEvents(cedUsuario, filters) {
        if (!cedUsuario?.trim()) {
    ...
    async getOrganizerEvents(cedOrganizador, filters) {
        if (!cedOrganizador?.trim()) {
            throw new Error("Cédula de organizador requerida");
        }
        const organizerExists = await this.userRepository.existsByCedula(cedOrganizador);
        if (!organizerExists) {
    ...
    async updateEvent(id, data) {
        ...
        if (data.nom_eve) {
            const nameExists = await this.eventRepository.existsByName(data.nom_eve, id);
            if (nameExists) {
                throw new Error("Ya existe un evento con ese nombre");
            }
        }
    ...
    async validateOrganizerPermissions(eventId, cedOrganizador) {
...
//# sourceMappingURL=EventManagementService.js.map