...
export interface PDFGenerationService {
     ...
     */
    generateCertificatePDF(templateName: string, certificateData: {
    ...
    }, digitalSignature: DigitalSignature): Promise<PDFGenerationResult>;
    /**
     ...
     */
    htmlToPDF(htmlContent: string, options?: {
     ...
     */
    addWatermark(filePath: string, watermark: {
        text: string;
        opacity: number;
        rotation: number;
        fontSize: number;
     ...
     */
    protectPDF(filePath: string, userPassword?: string, ownerPassword?: string, permissions?: {
        printing: boolean;
        modifying: boolean;
        copying: boolean;
        annotating: boolean;
     ...
     */
    validateTemplate(templateId: string, data: Record<string, any>): Promise<{
        isValid: boolean;
