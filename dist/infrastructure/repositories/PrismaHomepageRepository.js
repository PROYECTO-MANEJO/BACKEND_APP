...
exports.PrismaHomepageRepository = void 0;
const HomepageContent_1 = require("../../domain/entities/HomepageContent");
/**
 * Implementación del repositorio de Homepage usando Prisma
 * Sigue los principios SOLID y Clean Architecture
 */
class PrismaHomepageRepository {
    constructor(prisma) {
        this.prisma = prisma;
    }
    /**
    ...
    async createDefaultContent() {
        try {
            const defaultData = {
                ...
                descripcion_seccion1: "Ofrecemos una amplia variedad de cursos técnicos y académicos diseñados para potenciar tu desarrollo profesional.",
                ...
                descripcion_seccion3: "Obtén certificados oficiales que validen tus conocimientos y habilidades adquiridas.",
                titulo_seccion4: "Comunidad Académica",
                descripcion_seccion4: "Forma parte de una comunidad universitaria comprometida con la excelencia educativa.",
            ...
            console.error("[PrismaHomepageRepository.createDefaultContent] Error:", error);
     ...
     */
    async updateContent(contentData, editorId) {
        try {
            ...
            if (!existingContent) {
                existingContent = await this.prisma.paginaPrincipal.create({
                    data: {
                        ...
                        subtitulo_hero: contentData.heroSubtitle || "Sistema Integral de Gestión",
                        descripcion_hero: contentData.heroDescription ||
                            "Facultad de Ingeniería en Sistemas",
                        titulo_ofrecemos: contentData.offerTitle || "¿Qué Ofrecemos?",
                        subtitulo_ofrecemos: contentData.offerSubtitle || "Oportunidades de crecimiento",
                        titulo_seccion1: "Cursos Especializados",
     ...
     */
    async getImageByType(imageType) {
        try {
            const content = await this.prisma.paginaPrincipal.findFirst();
            if (!content) {
                return null;
            }
            const imageField = `imagen_${imageType}`;
            const imageBuffer = content[imageField];
            if (!imageBuffer) {
                return null;
            }
            return {
                ...
                uploadedAt: content.fecha_ultima_actualizacion || content.fecha_creacion,
     ...
     * Actualizar imagen
     */
    async updateImage(imageType, imageBuffer, mimeType, uploadedBy) {
        try {
    ...
    async getUpdateHistory(limit = 10) {
        try {
            ...
            return [
                {
                    id: content.id_pag,
                    updatedAt: content.fecha_ultima_actualizacion || content.fecha_creacion,
            ...
            console.error("[PrismaHomepageRepository.getUpdateHistory] Error:", error);
            return [];
        }
    }
    /**
    ...
    async getContentMetadata() {
        try {
            ...
            return {
                lastUpdate: content.fecha_ultima_actualizacion || content.fecha_creacion,
            ...
            console.error("[PrismaHomepageRepository.getContentMetadata] Error:", error);
            return null;
        }
    }
    /**
     ...
     */
    mapPrismaToEntity(prismaContent) {
        const sections = [
            {
                id: "1",
        ...
        imageTypes.forEach((type) => {
            ...
            if (imageBuffer) {
                images[type] = {
                    ...
                    buffer: imageBuffer,
                    mimeType: "image/jpeg",
                    uploadedAt: prismaContent.fecha_ultima_actualizacion ||
        ...
        const contentData = {
            ...
            lastUpdateDate: prismaContent.fecha_ultima_actualizacion ||
                prismaContent.fecha_creacion,
            createdAt: prismaContent.fecha_creacion,
            version: 1,
            isPublished: true,
        };
        return new HomepageContent_1.HomepageContent(contentData);
    }
    /**
     * Crear contenido por defecto en Prisma (método auxiliar)
     */
    async createDefaultPrismaContent() {
        return await this.prisma.paginaPrincipal.create({
            data: {
                titulo_hero: "FISEI - SIGEC",
                subtitulo_hero: "Sistema Integral de Gestión de Eventos y Cursos",
                descripcion_hero: "Facultad de Ingeniería en Sistemas, Electrónica e Industrial",
                titulo_ofrecemos: "¿Qué Ofrecemos?",
                subtitulo_ofrecemos: "Oportunidades de crecimiento académico y profesional",
                titulo_seccion1: "Cursos Especializados",
                descripcion_seccion1: "Cursos técnicos y académicos de calidad",
                titulo_seccion2: "Eventos Académicos",
                descripcion_seccion2: "Conferencias y seminarios especializados",
...
exports.PrismaHomepageRepository = PrismaHomepageRepository;
//# sourceMappingURL=PrismaHomepageRepository.js.map