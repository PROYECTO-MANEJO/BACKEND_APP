...
class PrismaReportRepository {
     ...
     */
    async generateEventEnrollmentReport(eventId) {
        try {
            ...
            const approvedEnrollments = event.inscripciones.filter((i) => i.estado_pago === "APROBADO").length;
            const pendingEnrollments = event.inscripciones.filter((i) => i.estado_pago === "PENDIENTE").length;
            const rejectedEnrollments = event.inscripciones.filter((i) => i.estado_pago === "RECHAZADO").length;
            // Agrupar inscripciones por fecha
            const enrollmentsByDate = this.groupEnrollmentsByDate(event.inscripciones);
            // Agrupar inscripciones por carrera
            const enrollmentsByCareer = this.groupEnrollmentsByCareer(event.inscripciones);
     ...
     */
    async generateCourseEnrollmentReport(courseId) {
        try {
            ...
            const approvedEnrollments = course.inscripcionesCurso.filter((i) => i.estado_pago_cur === "APROBADO").length;
            const pendingEnrollments = course.inscripcionesCurso.filter((i) => i.estado_pago_cur === "PENDIENTE").length;
            const rejectedEnrollments = course.inscripcionesCurso.filter((i) => i.estado_pago_cur === "RECHAZADO").length;
            const enrollmentsByDate = this.groupCourseEnrollmentsByDate(course.inscripcionesCurso);
            const enrollmentsByCareer = this.groupCourseEnrollmentsByCareer(course.inscripcionesCurso);
            const enrollmentDetails = course.inscripcionesCurso.map((inscription) => ({
     ...
     */
    async generateEventParticipationReport(eventId) {
        try {
            ...
            const completedParticipants = participations.filter((p) => p.aprobado).length;
            const failedParticipants = totalParticipants - completedParticipants;
            const averageAttendance = participations.length > 0
                ? participations.reduce((sum, p) => sum + p.asi_par, 0) /
                    participations.length
                : 0;
            const certificatesIssued = participations.filter((p) => p.certificado_pdf).length;
     ...
     */
    async generateCourseParticipationReport(courseId) {
     ...
     */
    async generateEventsOverviewReport(filters) {
        try {
            const whereClause = {};
            if (filters?.dateFrom || filters?.dateTo) {
                whereClause.fec_ini_eve = {};
                if (filters.dateFrom)
                    whereClause.fec_ini_eve.gte = filters.dateFrom;
                if (filters.dateTo)
                    whereClause.fec_ini_eve.lte = filters.dateTo;
            }
            if (filters?.categoryId)
                whereClause.id_cat_eve = filters.categoryId;
            if (filters?.organizerId)
                whereClause.ced_org_eve = filters.organizerId;
            const [totalEvents, activeEvents, totalEnrollments, events] = await Promise.all([
     ...
     */
    async generateCoursesOverviewReport(filters) {
    ...
    async getDashboardData() {
        try {
            const [totalUsers, totalEvents, totalCourses, totalEnrollments] = await Promise.all([
    ...
    async generateUsersByCareerReport() {
        throw new Error("Method not implemented");
    }
    ...
    // Métodos auxiliares
    groupEnrollmentsByDate(inscriptions) {
    ...
    groupEnrollmentsByCareer(inscriptions) {
        const grouped = inscriptions.reduce((acc, inscription) => {
    ...
    groupCourseEnrollmentsByDate(inscriptions) {
        const grouped = inscriptions.reduce((acc, inscription) => {
    ...
    groupCourseEnrollmentsByCareer(inscriptions) {
        const grouped = inscriptions.reduce((acc, inscription) => {
            const career = inscription.usuario.carrera;
            const key = career ? career.id_car : "sin-carrera";
            const name = career ? career.nom_car : "Sin carrera";
    ...
    groupEventsByCareer(events) {
        const grouped = events.reduce((acc, event) => {
            event.eventosPorCarrera.forEach((epc) => {
