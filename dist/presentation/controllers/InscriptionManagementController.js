...
exports.InscriptionManagementController = void 0;
const BaseController_1 = require("./BaseController");
class InscriptionManagementController extends BaseController_1.BaseController {
     ...
     */
    async getPendingEventInscriptions(req, res) {
        try {
     ...
     */
    async getPendingCourseInscriptions(req, res) {
        try {
            const { page = 1, limit = 10, search = '' } = req.query;
            const prisma = this.container.getPrismaClient();
            const skip = (Number(page) - 1) * Number(limit);
            ...
            if (search) {
                where.OR = [
                    ...
                    {
                        curso: {
                            nom_cur: { contains: search, mode: 'insensitive' }
                        }
                    }
                ];
            }
     ...
     */
    async approveEventInscription(req, res) {
        try {
            const { id } = req.params;
            const prisma = this.container.getPrismaClient();
     ...
     */
    async approveCourseInscription(req, res) {
        try {
            const { id } = req.params;
            const prisma = this.container.getPrismaClient();
            const adminId = req.usuario?.id_usu || req.uid;
     ...
     */
    async rejectEventInscription(req, res) {
        try {
            const { id } = req.params;
     ...
     */
    async rejectCourseInscription(req, res) {
        try {
            const { id } = req.params;
            const { motivo } = req.body;
            const prisma = this.container.getPrismaClient();
     ...
     */
    async downloadEventReceipt(req, res) {
        try {
            const { id } = req.params;
            const prisma = this.container.getPrismaClient();
            const inscripcion = await prisma.inscripcion.findUnique({
     ...
     */
    async downloadCourseReceipt(req, res) {
        try {
            const { id } = req.params;
            const prisma = this.container.getPrismaClient();
            const inscripcion = await prisma.inscripcionCurso.findUnique({
     ...
     */
    async getInscriptionStats(req, res) {
        try {
            const prisma = this.container.getPrismaClient();
            const [eventStats, courseStats, recentInscriptions] = await Promise.all([
     ...
     */
    async getCoursesAndEventsForManagement(req, res) {
        try {
            const prisma = this.container.getPrismaClient();
            ...
            const eventosConEstadisticas = await Promise.all(eventos.map(async (evento) => {
            ...
            const cursosConEstadisticas = await Promise.all(cursos.map(async (curso) => {
     ...
     */
    async getEventDetailsForAdmin(req, res) {
        try {
     ...
     */
    async getCourseDetailsForAdmin(req, res) {
        try {
...
exports.InscriptionManagementController = InscriptionManagementController;
//# sourceMappingURL=InscriptionManagementController.js.map