...
class ReportsController extends BaseController_1.BaseController {
     ...
     */
    async generateCoursesReport(req, res) {
        try {
            const prisma = this.container.getPrismaClient();
     ...
     */
    async generateChangeRequestsStatusReport(req, res) {
        try {
     ...
     */
    async generateChangeRequestsDevelopersReport(req, res) {
        try {
            // Verificar que sea MASTER
            const userRole = await this.getAuthenticatedUserRole(req);
            if (userRole !== 'MASTER') {
            ...
            // 🖨️ Generar PDF
            const pdfBuffer = await this.generateChangeRequestsDevelopersPDF(resultado, desarrolladores, solicitudesDetalle, sinAsignar);
     ...
     */
    async generateChangeRequestsSummaryReport(req, res) {
        try {
            // Verificar que sea MASTER
            const userRole = await this.getAuthenticatedUserRole(req);
            if (userRole !== 'MASTER') {
            ...
            const [totalSolicitudes, porEstado, porPrioridad, porTipo, solicitudesHoy, solicitudesEsteMes, desarrolladoresInfo] = await Promise.all([
                // Total general
    ...
    async getReportStats(req, res) {
        try {
            const prisma = this.container.getPrismaClient();
            const [totalReportes, reportesPorTipo, reportesRecientes] = await Promise.all([
    ...
    // =====================================================
    async generateFinancialPDF(data) {
    ...
    async generateUsersPDF(usuarios) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const chunks = [];
    ...
    async generateEventsPDF(agrupados) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const chunks = [];
    ...
    async generateCoursesPDF(agrupados) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const chunks = [];
    ...
    async generateChangeRequestsStatusPDF(reportePorEstado, solicitudesDetalle) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const chunks = [];
    ...
    async generateChangeRequestsDevelopersPDF(resultado, desarrolladores, solicitudesDetalle, sinAsignar) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const chunks = [];
    ...
    async generateChangeRequestsSummaryPDF(data) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const chunks = [];
    ...
    // MÉTODOS AUXILIARES
    // =====================================================
    async getAuthenticatedUserRole(req) {
        try {
