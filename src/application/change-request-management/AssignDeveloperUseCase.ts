...
export class AssignDeveloperUseCase {
  ...
  public async execute(data: AssignDeveloperDTO): Promise<ChangeRequest> {
    ...
    if (changeRequest.status !== "APROBADA") {
      throw new Error(
        "Solo se pueden asignar desarrolladores a solicitudes aprobadas"
      );
    }
    ...
    if (!developer.canTakeNewRequest()) {
      throw new Error(
        `El desarrollador no puede tomar más solicitudes. Carga actual: ${developer.getCurrentWorkload()}`
      );
    }
