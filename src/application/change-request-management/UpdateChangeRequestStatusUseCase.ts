...
export class UpdateChangeRequestStatusUseCase {
  ...
  public async execute(data: UpdateChangeRequestStatusDTO): Promise<ChangeRequest> {
    ...
    if (data.comments && data.comments.trim().length > 0 && data.newStatus !== 'RECHAZADA') {
  ...
  private validateInput(data: UpdateChangeRequestStatusDTO): void {
    ...
    const validStatuses = [
      ...
      'ESPERANDO_INFORMACION', 'EN_DESARROLLO', 'EN_TESTING', 'EN_PAUSA',
      'COMPLETADA', 'CERRADA', 'CANCELADA'
    ];
