...
import { HomepageValidator } from "../../domain/validators/HomepageValidator";
...
export class HomepageService {
  ...
  async getHomepageContent(): Promise<HomepageServiceResponse> {
    try {
      ...
      return {
        success: false,
        message:
          error.message || "Error al obtener el contenido de la homepage",
      };
    }
  }
  ...
  async updateHomepageContent(
    contentRequest: HomepageContentRequest
    ...
    try {
      ...
      const updatedContent = await homepageRepository.updateContent(
        contentData,
        contentRequest.editor_id || ""
      );
  ...
  async updateHomepageImage(
    imageRequest: HomepageImageRequest
  ): Promise<HomepageServiceResponse> {
    try {
      // ✅ SRP: Delegar validación al HomepageValidator
      HomepageValidator.validateImageUpdate(
      ...
      // Actualizar imagen
      await homepageRepository.updateImage(
        imageRequest.imageType,
        imageRequest.imageFile.buffer,
        imageRequest.imageFile.mimetype,
        imageRequest.editor_id || ""
      );
   ...
   */
  async getHomepageImage(imageType: string): Promise<HomepageServiceResponse> {
    try {
   ...
   */
  async getStudentContent(userId: string): Promise<HomepageServiceResponse> {
    try {
      const prisma = this.container.getPrismaClient();
   ...
   */
  async getExternalContent(): Promise<HomepageServiceResponse> {
    try {
      const prisma = this.container.getPrismaClient();
