...
export class GenerateCertificateUseCase {
      ...
      if (request.participationType === "event") {
        if (!request.participationId) {
          return {
            success: false,
            message:
              "ID de participación es requerido para certificados de eventos",
          };
        }

        certificate =
          await this.certificateManagementService.generateEventParticipationCertificate(
        ...
        if (!request.completionId) {
          return {
            success: false,
            message:
              "ID de finalización es requerido para certificados de cursos",
          };
        }

        certificate =
          await this.certificateManagementService.generateCourseCompletionCertificate(
  ...
  private validateRequest(request: GenerateCertificateRequest): string[] {
    const errors: string[] = [];
    ...
    if (
      request.participationType === "event" &&
    ...
    if (
      request.participationType === "course" &&
