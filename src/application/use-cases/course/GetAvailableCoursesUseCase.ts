...
export class GetAvailableCoursesUseCase {
  ...
  async execute(request: GetAvailableCoursesRequest = {}): Promise<GetAvailableCoursesResponse> {
    try {
      ...
      if (request.categoryId) {
        filteredCourses = filteredCourses.filter(course => 
          course.id_cat_cur === request.categoryId
        );
      }

      if (request.isFree !== undefined) {
        filteredCourses = filteredCourses.filter(course => 
          course.es_gratuito === request.isFree
        );
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Error desconocido al obtener cursos disponibles'
      };
    }
  }
}