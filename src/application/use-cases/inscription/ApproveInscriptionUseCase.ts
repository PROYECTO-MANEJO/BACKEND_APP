...
export class ApproveInscriptionUseCase {
  ...
  async execute(request: ApproveInscriptionRequest): Promise<ApproveInscriptionResponse> {
    try {
      ...
      // Aprobar inscripción
      const approvedInscription = await this.inscriptionManagementService.approveInscription(
