...
export class EnrollInCourseUseCase {
  ...
  async execute(request: EnrollInCourseRequest): Promise<EnrollInCourseResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Error desconocido al inscribir en curso'
      };
    }
  }
}