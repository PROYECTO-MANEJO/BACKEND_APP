...
export class GetPendingInscriptionsUseCase {
  ...
  async execute(
    request: GetPendingInscriptionsRequest = {}
  ): Promise<GetPendingInscriptionsResponse> {
    try {
      // Obtener todas las inscripciones pendientes
      const pendingInscriptions =
        await this.inscriptionManagementService.getPendingInscriptions();
      ...
      const statistics = {
        ...
        eventInscriptions: pendingInscriptions.filter((ins) => ins.isForEvent())
          .length,
        courseInscriptions: pendingInscriptions.filter((ins) =>
          ins.isForCourse()
        ).length,
      };
      ...
      if (request.page && request.limit) {
        ...
        return {
          success: true,
          inscriptions: paginatedInscriptions.map((ins) =>
            ins.toPublicObject()
          ),
          pagination: {
