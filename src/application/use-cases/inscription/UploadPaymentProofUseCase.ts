...
export class UploadPaymentProofUseCase {
  ...
  async execute(request: UploadPaymentProofRequest): Promise<UploadPaymentProofResponse> {
    try {
      ...
      const updatedInscription = await this.inscriptionManagementService.updatePaymentProof(
