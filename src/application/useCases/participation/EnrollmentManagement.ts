...
export interface EnrollmentRepository {
  ...
  getTopActivities(limit: number): Promise<
    Array<{
      activityId: string;
      activityName: string;
...
export class EnrollmentManagement {
  ...
  async createEnrollment(input: EnrollmentManagementInput): Promise<{
    ...
    try {
      ...
      // Validate activity
      const activity = await this.activityService.getActivityDetails(
        input.activityId
      );
      if (!activity) {
        return {
      ...
      const isAvailable = await this.activityService.isActivityAvailable(
        input.activityId
      );
      if (!isAvailable) {
        return {
      ...
      const existingEnrollments =
        await this.enrollmentRepository.findByParticipantId(
          input.participantId
        );
      ...
      const hasCapacity = await this.activityService.hasAvailableCapacity(
        input.activityId
      );
      let waitingListPosition: number | undefined;

      if (!hasCapacity) {
        const waitingList = await this.enrollmentRepository.findWaitingList(
          input.activityId
        );
  ...
  async getEnrollments(filters?: ParticipationFilters): Promise<{
    ...
    try {
      ...
      const enrollmentSummaries = enrollments.map((enrollment) =>
        enrollment.getEnrollmentSummary()
      );
  ...
  async getActivityCapacity(activityId: string): Promise<{
    ...
    try {
      const capacity = await this.enrollmentRepository.getActivityCapacity(
        activityId
      );
      ...
      return {
        success: false,
        message: `Failed to retrieve activity capacity: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
      };
    }
  ...
  async getWaitingListManagement(activityId: string): Promise<{
    ...
    try {
      const waitingListEnrollments =
        await this.enrollmentRepository.findWaitingList(activityId);
      const capacity = await this.enrollmentRepository.getActivityCapacity(
        activityId
      );
      ...
      return {
        success: false,
        message: `Failed to retrieve waiting list management: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
      };
    }
  ...
  private async reorderWaitingList(
    enrollmentId: string,
    newPosition: number
  ): Promise<void> {
    const enrollment = await this.enrollmentRepository.findById(enrollmentId);
    ...
    for (let i = 0; i < sortedList.length; i++) {
      ...
      if (enrollment) {
        ...
        if (currentPosition !== newPosition) {
          const reorderedEnrollment = enrollment.addToWaitingList(
            newPosition,
            enrollment.getPriorityScore()
          );
  ...
  private async notifyWaitingList(
    ...
    const enrollmentsToNotify = upToPosition
      ? waitingList.filter(
          (e) => (e.getWaitingListPosition() || 0) <= upToPosition
        )
      : waitingList;
