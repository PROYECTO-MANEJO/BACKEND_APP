...
export class ApproveInscriptionUseCase {
  ...
  private validateRequest(request: ApproveInscriptionRequest): void {
    ...
    if (
      request.approvalData.paymentMethod &&
      !["EFECTIVO", "TARJETA", "TRANSFERENCIA", "BECA"].includes(
        request.approvalData.paymentMethod
      )
    ) {
    ...
    if (
      request.approvalData.paymentAmount &&
    ...
    if (
      request.approvalData.scholarshipType &&
    ...
    if (request.approvalData.scholarshipPercentage) {
      if (
        request.approvalData.scholarshipPercentage < 0 ||
    ...
    if (
      request.approvalData.scholarshipReason &&
    ...
    if (
      request.approvalData.additionalNotes &&
