...
import {
  ParticipationType,
  ParticipationStatus,
} from "../../../domain/entities/administration";
...
export interface IParticipationRegistrationRepository {
  ...
  findByUserAndActivity(
    ...
    activityId: string,
    activityType: string
  ): Promise<ParticipationRegistration | null>;
  save(
    participation: ParticipationRegistration
...
export class RegisterParticipationUseCase {
  ...
  public async execute(
    ...
    try {
      ...
      const existingParticipation =
        await this.participationRepo.findByUserAndActivity(
          request.userId,
          request.activityId,
      ...
      if (existingParticipation) {
        return await this.updateExistingParticipation(
      ...
      const activityInfo = await this.activityRepo.findById(
        request.activityId,
        request.activityType
      );
      ...
      if (request.attendanceData?.checkInTime) {
        ...
        if (request.attendanceData.notes) {
          updatedParticipation = updatedParticipation.addNote(
            request.attendanceData.notes
          );
        }
      }

      // Guardar participación
      const savedParticipation = await this.participationRepo.save(
      ...
      try {
        ...
        console.error(
          "Failed to send participation notification:",
          notificationError
        );
      }
  ...
  private async updateExistingParticipation(
    ...
    if (request.attendanceData?.checkInTime) {
      ...
      if (request.attendanceData.notes) {
        updatedExistingParticipation = updatedExistingParticipation.addNote(
          request.attendanceData.notes
        );
      }
    ...
    const certificateEligible =
      updatedExistingParticipation.canGenerateCertificate();
    const certificateGenerated =
      updatedExistingParticipation.isCertificateGenerated();
    ...
    return {
      ...
      certificateGenerated,
      progressPercentage:
        updatedExistingParticipation.getCompletionPercentage(),
  ...
  private validateRequest(request: RegisterParticipationRequest): void {
    ...
    if (
      !["PRESENCIAL", "VIRTUAL", "HIBRIDA"].includes(request.participationType)
    ) {
    ...
    if (request.attendanceData) {
      if (
        request.attendanceData.attendancePercentage &&
  ...
  private calculateNextSteps(
    ...
    switch (currentStatus) {
      ...
      case "ATTENDED":
        if (!certificateEligible) {
          steps.push(
            "Complete additional requirements for certificate eligibility"
          );
