...
export class CreateRepositoryUseCase {
  ...
  public async execute(
    ...
    try {
      ...
      const existingRepo = await this.repositoryRepo.findByName(
        request.name,
      ...
      const repository = GitHubRepository.fromGitHubAPI(
        githubRepoData,
        repositoryType
      );
  ...
  private validateRequest(request: CreateRepositoryRequest): void {
    ...
    if (request.name.length > 100) {
      throw new Error(
        "El nombre del repositorio no puede exceder 100 caracteres"
      );
    }
