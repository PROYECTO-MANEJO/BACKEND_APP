...
export class SyncRepositoryUseCase {
  ...
  public async execute(
    ...
    try {
      ...
      const githubRepoData = await this.githubAPI.getRepository(
        repositoryName,
      ...
      if (request.syncBranches !== false) {
        const branchStats = await this.syncBranches(
          repository.getId(),
          repositoryFullName
        );
      ...
      if (request.syncPullRequests !== false) {
        const prStats = await this.syncPullRequests(
          repository.getId(),
          repositoryFullName
        );
      ...
      if (request.syncIssues !== false) {
        const issueStats = await this.syncIssues(
          repository.getId(),
          repositoryFullName
        );
      ...
      if (request.syncCommits !== false) {
        const commitStats = await this.syncCommits(
          repository.getId(),
          repositoryFullName,
          request.maxCommits
        );
  ...
  private async syncRepository(
    ...
    try {
      ...
      const repositoryType = this.determineRepositoryType(
        githubRepoData.name,
      ...
      if (existingId) {
        ...
        if (existingRepo) {
          const updatedRepo = GitHubRepository.fromGitHubAPI(
            githubRepoData,
            repositoryType
          );
      ...
      const newRepo = GitHubRepository.fromGitHubAPI(
        githubRepoData,
        repositoryType
      );
      ...
      throw new Error(
        `Error sincronizando repositorio: ${
          error instanceof Error ? error.message : "Error desconocido"
        }`
      );
    }
  ...
  private async syncBranches(
    ...
    try {
      const githubBranches = await this.githubAPI.listBranches(
        repositoryFullName
      );
        ...
        try {
          ...
          if (existingBranch) {
            ...
            if (existingBranch.getSha() !== branchData.commit?.sha) {
              const updatedBranch = GitHubBranch.fromGitHubAPI(
                branchData,
                repositoryId,
                repositoryFullName
              );
            ...
            // Crear nueva rama
            const newBranch = GitHubBranch.fromGitHubAPI(
              branchData,
              repositoryId,
              repositoryFullName
            );
  ...
  private async syncPullRequests(
    ...
    try {
      ...
      const openPRs = await this.githubAPI.listPullRequests(
        repositoryFullName,
        "open"
      );
      const closedPRs = await this.githubAPI.listPullRequests(
        repositoryFullName,
        "closed"
      );
      ...
      for (const prData of allPRs) {
        try {
          const existingPR = await this.pullRequestRepo.findByNumber(
            prData.number,
            repositoryId
          );
          ...
          if (existingPR) {
            ...
            if (existingPR.getUpdatedAt() < currentUpdatedAt) {
              const updatedPR = GitHubPullRequest.fromGitHubAPI(
                prData,
                repositoryId
              );
  ...
  private async syncIssues(
    ...
    try {
      ...
      const openIssues = await this.githubAPI.listIssues(
        repositoryFullName,
        "open"
      );
      const closedIssues = await this.githubAPI.listIssues(
        repositoryFullName,
        "closed"
      );
      ...
      for (const issueData of allIssues) {
        try {
          ...
          const existingIssue = await this.issueRepo.findByNumber(
            issueData.number,
            repositoryId
          );
          ...
          if (existingIssue) {
            ...
            if (existingIssue.getUpdatedAt() < currentUpdatedAt) {
              const updatedIssue = GitHubIssue.fromGitHubAPI(
                issueData,
                repositoryId
              );
          ...
          errors++;
          console.warn(
            `Error sincronizando issue #${issueData.number}:`,
            error
          );
        }
      }
  ...
  private async syncCommits(
    ...
    try {
      const commits = await this.githubAPI.listCommits(
        repositoryFullName,
        undefined,
        maxCommits
      );
      ...
      for (const commitData of commits) {
        try {
          const existingCommit = await this.commitRepo.findBySha(
            commitData.sha,
            repositoryId
          );
          ...
          if (existingCommit) {
            ...
            // Crear nuevo commit
            const newCommit = GitHubCommit.fromGitHubAPI(
              commitData,
              repositoryId
            );
  ...
  private async getBasicRepository(
    request: SyncRepositoryRequest
  ): Promise<GitHubRepository> {
  ...
  private validateRequest(request: SyncRepositoryRequest): void {
    ...
    if (request.repositoryName) {
      ...
      if (!namePattern.test(request.repositoryName)) {
        throw new Error(
          "El nombre del repositorio contiene caracteres inválidos"
        );
      }
    }
    ...
    if (request.organization) {
      ...
      if (!orgPattern.test(request.organization)) {
        throw new Error(
          "El nombre de la organización contiene caracteres inválidos"
        );
      }
    }
  }
