...
export class Enrollment {
  ...
  private static fromEventInscription(inscription: any): Enrollment {
    ...
    const enrollmentData: EnrollmentData = {
      ...
      approvalStatus:
        (inscription.aprobacion_estado as
          | "PENDING"
          | "APPROVED"
          | "REJECTED") || "APPROVED",
  ...
  private static fromCourseInscription(inscripcionCurso: any): Enrollment {
    ...
    const enrollmentData: EnrollmentData = {
      ...
      approvalStatus:
        (inscripcionCurso.aprobacion_estado_cur as
          | "PENDING"
          | "APPROVED"
          | "REJECTED") || "APPROVED",
  ...
  public isOnWaitingList(): boolean {
    return (
      this.data.hasWaitingList && this.data.waitingListPosition !== undefined
    );
  }
  ...
  public isApproved(): boolean {
    return (
      !this.data.requiresApproval || this.data.approvalStatus === "APPROVED"
    );
  }

  public isApprovalRejected(): boolean {
    return (
      this.data.requiresApproval && this.data.approvalStatus === "REJECTED"
    );
  }
  ...
  public canProcessPayment(): boolean {
    return (
      this.data.paymentStatus === "PENDING" && this.data.status !== "CANCELLED"
    );
  }
  ...
  public updatePaymentStatus(
    ...
    const updatedData = {
      ...
      confirmationDate:
        newStatus === "CONFIRMED" ? new Date() : this.data.confirmationDate,
      updatedAt: new Date(),
  ...
  public getRequiredDocumentsStatus() {
    return {
      ...
      pending: this.data.requiredDocuments.filter(
        (doc) => !this.data.submittedDocuments.includes(doc)
      ),
