...
export class CareerManagementService {
  ...
  async createCareer(careerData: CreateCareerDto): Promise<{ success: boolean; message: string; career?: Career }> {
    try {
      // Validar datos
      if (!careerData.name || !careerData.code || !careerData.faculty) {
        return { success: false, message: 'Nombre, código y facultad son requeridos' };
      }

      // Validar código
      if (!CareerEntity.isValidCode(careerData.code)) {
        return { success: false, message: 'El código debe tener entre 2 y 10 caracteres' };
      }

      // Verificar si el código ya existe
      const codeExists = await this.careerRepository.codeExists(careerData.code);
      if (codeExists) {
        return { success: false, message: 'Ya existe una carrera con ese código' };
      }
  ...
  async updateCareer(id: number, updateData: UpdateCareerDto): Promise<{ success: boolean; message: string; career?: Career }> {
    try {
      ...
      if (updateData.code) updatePayload.code = updateData.code.trim().toUpperCase();
  ...
  async toggleCareerStatus(id: number): Promise<{ success: boolean; message: string }> {
    try {
  ...
  async deleteCareer(id: number): Promise<{ success: boolean; message: string }> {
    try {
      const career = await this.careerRepository.findById(id);
      if (!career) {
        return { success: false, message: 'Carrera no encontrada' };
