...
export class CourseManagementService {
  ...
  async createCourse(courseData: CourseData): Promise<Course> {
    ...
    const conflictingCourses = await this.courseRepository.findConflictingCourses(
   ...
   * ✅ ACTUALIZAR CURSO
   */
  async updateCourse(courseId: string, updateData: Partial<CourseData>): Promise<Course> {
   ...
   */
  async updateCourseCarerIds(courseId: string, careerIds: number[], organizerId: string): Promise<Course> {
    // 1. Obtener curso
    const course = await this.courseRepository.findById(courseId);
    if (!course) {
      throw new Error('Curso no encontrado');
   ...
   */
  async canUserEnroll(courseId: string, userId: string): Promise<{ canEnroll: boolean; reason?: string }> {
    // 1. Obtener curso
    const course = await this.courseRepository.findById(courseId);
    if (!course) {
   ...
   */
  async validateScheduleConflicts(organizerId: string, startDate: Date, endDate: Date, excludeCourseId?: string): Promise<Course[]> {
