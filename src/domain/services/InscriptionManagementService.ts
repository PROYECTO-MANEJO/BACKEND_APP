...
import { Inscription, InscriptionData, InscriptionType, PaymentMethod } from '../entities/Inscription';
...
export class InscriptionManagementService {
  ...
  async enrollInEvent(data: {
    ...
    const existingInscription = await this.inscriptionRepository.findByUserAndTarget(
      data.userId,
      data.eventId,
      'EVENT'
    );
   ...
   * ✅ APROBAR INSCRIPCIÓN
   */
  async approveInscription(inscriptionId: string, approverUserId: string): Promise<Inscription> {
   ...
   */
  async cancelInscription(inscriptionId: string, userId: string): Promise<Inscription> {
    // 1. Obtener inscripción
    const inscription = await this.inscriptionRepository.findById(inscriptionId);
    if (!inscription) {
      throw new Error('Inscripción no encontrada');
   ...
   */
  async getUserInscriptions(userId: string, filters?: {
    type?: InscriptionType;
    status?: string;
  }): Promise<Inscription[]> {
   ...
   */
  async getInscriptionStatistics(targetId: string, type: InscriptionType): Promise<{
