...
export class UserManagementService {
  ...
  async updateUserProfile(userId: number, updateData: UpdateUserProfileDto): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (updateData.careerId) {
        const careerExists = await this.careerRepository.existsAndActive(updateData.careerId);
        if (!careerExists) {
          return { success: false, message: 'La carrera seleccionada no existe o no está activa' };
        }
      }
      ...
      if (updateData.githubToken) {
        ...
        if (!userProfile || !allowedRoles.includes(userProfile.account.role)) {
          return { 
            success: false, 
            message: 'Solo desarrolladores, masters y administradores pueden configurar token de GitHub' 
          };
        }
      }
      ...
      if (updateData.secondName !== undefined) profileData.secondName = updateData.secondName;
      if (updateData.lastName) profileData.lastName = updateData.lastName;
      if (updateData.secondLastName !== undefined) profileData.secondLastName = updateData.secondLastName;
      if (updateData.dateOfBirth) profileData.dateOfBirth = updateData.dateOfBirth;
      if (updateData.phoneNumber !== undefined) profileData.phoneNumber = updateData.phoneNumber;
      if (updateData.careerId !== undefined) profileData.careerId = updateData.careerId;
      if (updateData.githubToken !== undefined) profileData.githubToken = updateData.githubToken;

      // Actualizar perfil
      const updated = await this.userRepository.updateProfile(userId, profileData);
      ...
      console.error('[UserManagementService] Error actualizando perfil:', error);
      return { success: false, message: 'Error interno del servidor' };
    }
  }

  async getAllUsers(page: number = 1, limit: number = 10, filters?: Partial<User>): Promise<{ users: UserProfileResponse[]; total: number; totalPages: number }> {
    try {
      const { users, total } = await this.userRepository.findPaginated(page, limit, filters);
      
      const userProfiles: UserProfileResponse[] = [];
      
      for (const user of users) {
        const completeProfile = await this.userRepository.getCompleteProfile(user.id);
        if (completeProfile) {
          userProfiles.push({
      ...
      console.error('[UserManagementService] Error obteniendo usuarios:', error);
