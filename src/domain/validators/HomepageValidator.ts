...
export class HomepageValidator {
   ...
   */
  public static validateFilters(data: HomepageValidationData): void {
    ...
    if (data.filtros?.modalidad) {
      ...
      if (!modalidadesValidas.includes(data.filtros.modalidad)) {
        throw new Error(
          `Modalidad inválida. Debe ser una de: ${modalidadesValidas.join(
            ", "
          )}`
        );
      }
   ...
   */
  public static validatePagination(data: HomepageValidationData): void {
   ...
   */
  public static validateSearchText(texto: string): void {
   ...
   */
  public static validateContentUpdate(
    content: HomepageContentValidation
  ): void {
    // Validar campos hero
    ...
    for (let i = 1; i <= 4; i++) {
      const titleKey = `titulo_seccion${i}` as keyof HomepageContentValidation;
      const descKey =
        `descripcion_seccion${i}` as keyof HomepageContentValidation;

      if (content[titleKey] !== undefined) {
        this.validateSectionTitle(content[titleKey] as string, i);
      }

      if (content[descKey] !== undefined) {
        this.validateSectionDescription(content[descKey] as string, i);
      }
    }
   ...
   */
  public static validateImageUpdate(imageType: string, imageFile?: any): void {
    ...
    if (!validImageTypes.includes(imageType)) {
      throw new Error(
        `Tipo de imagen no válido. Debe ser uno de: ${validImageTypes.join(
          ", "
        )}`
      );
    }
   ...
   */
  private static validateHeroTitle(title: string): void {
  ...
  private static validateHeroSubtitle(subtitle: string): void {
  ...
  private static validateHeroDescription(description: string): void {
  ...
  private static validateOfferTitle(title: string): void {
    if (typeof title !== "string") {
  ...
  private static validateOfferSubtitle(subtitle: string): void {
    if (typeof subtitle !== "string") {
    ...
    if (subtitle.length > 120) {
      throw new Error(
        "El subtítulo de ofrecemos no puede exceder 120 caracteres"
      );
    }
  }
  ...
  private static validateSectionTitle(
    ...
    if (typeof title !== "string") {
      throw new Error(
        `El título de la sección ${sectionNumber} debe ser una cadena de texto`
      );
    }

    if (title.length > 80) {
      throw new Error(
        `El título de la sección ${sectionNumber} no puede exceder 80 caracteres`
      );
    }
  }
  ...
  private static validateSectionDescription(
    ...
    if (typeof description !== "string") {
      throw new Error(
        `La descripción de la sección ${sectionNumber} debe ser una cadena de texto`
      );
    }

    if (description.length > 300) {
      throw new Error(
        `La descripción de la sección ${sectionNumber} no puede exceder 300 caracteres`
      );
    }
  }

  private static validateFooterText(text: string, footerNumber: number): void {
    if (typeof text !== "string") {
      throw new Error(
        `El texto del footer ${footerNumber} debe ser una cadena de texto`
      );
    }

    if (text.length > 200) {
      throw new Error(
        `El texto del footer ${footerNumber} no puede exceder 200 caracteres`
      );
    }
  }

  private static validateImageFile(file: any): void {
