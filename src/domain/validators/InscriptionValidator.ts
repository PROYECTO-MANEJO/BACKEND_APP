...
export class InscriptionValidator {
   ...
   */
  public static validate(data: InscriptionValidationData): void {
    ...
    if (data.id_cur && data.id_eve) {
      throw new Error(
        "No se puede inscribir tanto en curso como en evento simultáneamente"
      );
    }
    ...
    if (
      ...
      throw new Error(
        "El estado debe ser PENDIENTE, APROBADA, RECHAZADA o CANCELADA"
      );
    }
  }
   ...
   */
  public static validateRequiredDocuments(
    data: InscriptionValidationData,
    ...
    if (requiresMotivationLetter) {
      ...
      if (data.carta_motivacion.length < 100) {
        throw new Error(
          "La carta de motivación debe tener al menos 100 caracteres"
        );
      }
    }
  }
   ...
   */
  public static validateInscriptionPeriod(
    startDate: Date,
    endDate: Date
  ): void {
    const now = new Date();
