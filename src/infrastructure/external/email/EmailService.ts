...
export class EmailService implements IEmailService {
  ...
  private createTransporter(): nodemailer.Transporter {
    ...
    if (missingVars.length > 0) {
      console.warn(
        `[EmailService] Variables de entorno faltantes: ${missingVars.join(
          ", "
        )}`
      );
    }
  ...
  async sendVerificationEmail(email: string, token: string): Promise<boolean> {
    try {
      if (!process.env.FRONTEND_URL || !process.env.SMTP_FROM) {
        console.error(
          "[EmailService] Variables de entorno faltantes para verificación"
        );
        return false;
      }
      ...
      console.error(
        "[EmailService] Error enviando correo de verificación:",
        error
      );
      return false;
    }
  ...
  async sendPasswordResetEmail(email: string, token: string): Promise<boolean> {
    try {
      if (!process.env.FRONTEND_URL || !process.env.SMTP_FROM) {
        console.error(
          "[EmailService] Variables de entorno faltantes para recuperación"
        );
        return false;
      }
      ...
      console.error(
        "[EmailService] Error enviando correo de recuperación:",
        error
      );
      return false;
    }
