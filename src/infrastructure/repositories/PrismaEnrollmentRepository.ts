...
export class PrismaEnrollmentRepository {
  ...
  async findEventEnrollmentsByUser(userId: string): Promise<EnrollmentData[]> {
    ...
    return inscripciones.map((inscripcion) =>
      this.mapToEnrollmentData(inscripcion, "event")
    );
  }
  ...
  async findCourseEnrollmentsByUser(userId: string): Promise<EnrollmentData[]> {
    ...
    return inscripciones.map((inscripcion) =>
      this.mapToEnrollmentData(inscripcion, "course")
    );
  }

  async findEventEnrollmentsByEvent(
    eventId: string
  ): Promise<EnrollmentData[]> {
    const inscripciones = await this.prisma.inscripcion.findMany({
  ...
  async findCourseEnrollmentsByCourse(
    courseId: string
  ): Promise<EnrollmentData[]> {
    const inscripciones = await this.prisma.inscripcionCurso.findMany({
  ...
  async findEnrollmentsByPaymentStatus(
    status: string,
    type: "event" | "course"
  ): Promise<EnrollmentData[]> {
    if (type === "event") {
