 ...
 * Sincronizado con EventData del dominio para mantener consistencia
 */
...
export class PrismaEventRepository {
  ...
  async update(id: string, eventData: Partial<EventData>): Promise<EventData> {
  ...
  async findByCategory(categoryId: string): Promise<EventData[]> {
    const eventos = await this.prisma.evento.findMany({
  ...
  async findByStatus(status: string): Promise<EventData[]> {
    const eventos = await this.prisma.evento.findMany({
      where: { estado: status },
      include: {
        categoria: true,
  ...
  async findUpcoming(days?: number): Promise<EventData[]> {
    const now = new Date();
  ...
  async getEnrollmentCount(eventId: string): Promise<number> {
  ...
  async findByArea(area: string): Promise<EventData[]> {
    const eventos = await this.prisma.evento.findMany({
      where: { are_eve: area as any },
      include: {
        categoria: true,
        organizador: true,
        eventosPorCarrera: {
  ...
  private mapToEventData(evento: any): EventData {
    return {
      id: evento.id_eve,
      name: evento.nom_eve,
      ...
      associatedCareers:
        evento.eventosPorCarrera?.map((epc: any) => epc.id_car_per) || [],
    };
  }
}
