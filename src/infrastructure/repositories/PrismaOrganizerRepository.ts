...
import {
  ...
  OrganizerFilters,
} from "../../domain/repositories/IOrganizerRepository";
import {
  Organizador,
  CreateOrganizerDto,
  UpdateOrganizerDto,
  OrganizerWithRelations,
} from "../../domain/entities/Organizador";
 ...
 */
export class PrismaOrganizerRepository implements IOrganizerRepository {
  constructor(private prisma: PrismaClient) {}

  /**
   * Crear un nuevo organizador
   */
  async create(organizadorData: Partial<Organizador>): Promise<Organizador> {
    try {
   ...
   */
  async findByCedula(cedula: string): Promise<Organizador | null> {
    try {
   ...
   */
  async findById(id: string): Promise<Organizador | null> {
   ...
   */
  async findWithFilters(filters: OrganizerFilters): Promise<Organizador[]> {
    try {
   ...
   */
  async findEventsByCedula(cedula: string): Promise<any[]> {
    try {
      const organizer = await this.prisma.organizador.findUnique({
        where: { ced_org: cedula },
        include: {
   ...
   */
  async findCoursesByCedula(cedula: string): Promise<any[]> {
    try {
      const organizer = await this.prisma.organizador.findUnique({
        where: { ced_org: cedula },
        include: {
   ...
   */
  private mapPrismaToEntity(prismaOrganizer: any): Organizador {
    return {
