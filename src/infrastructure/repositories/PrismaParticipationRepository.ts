...
import {
  ...
  CertificateData,
  ParticipationStats,
} from "../../domain/repositories/IParticipationRepository";
import {
  ParticipationData,
  ParticipationType,
  ParticipationStatus,
...
export class PrismaParticipationRepository implements IParticipationRepository {
   ...
   */
  async create(
    participationData: Partial<ParticipationData>
   ...
   */
  async findById(id: string): Promise<ParticipationData | null> {
    try {
  ...
  async findByInscriptionId(
    ...
    try {
      ...
      throw new Error(
        `Error finding participation by inscription id: ${error}`
      );
    }
  }
   ...
   */
  async findAll(): Promise<ParticipationData[]> {
    try {
  ...
  async findWithFilters(
    ...
    try {
      ...
      if (
        filters.attendanceMin !== undefined ||
   ...
   */
  async findByUserId(userId: string): Promise<ParticipationData[]> {
    try {
      const participations = await this.prisma.participacion.findMany({
        where: {
          inscripcion: {
   ...
   */
  async findByEventId(eventId: string): Promise<ParticipationData[]> {
    try {
      const participations = await this.prisma.participacion.findMany({
        where: {
          inscripcion: {
   ...
   */
  async findApproved(): Promise<ParticipationData[]> {
    try {
   ...
   */
  async findPending(): Promise<ParticipationData[]> {
    try {
      const participations = await this.prisma.participacion.findMany({
        where: {
          fec_evaluacion: null,
   ...
   */
  async evaluateApproval(id: string): Promise<ParticipationData | null> {
    try {
   ...
   */
  private mapPrismaToEntity(prismaParticipation: any): ParticipationData {
    return {
      ...
      activityType: "EVENT" as ParticipationType,
      ...
      participantEmail:
        prismaParticipation.inscripcion.usuario?.cuentas?.[0]?.cor_cue || "",
      ...
      paymentStatus:
        prismaParticipation.inscripcion.estado_pago === "APROBADO"
          ? "APPROVED"
      ...
      status: prismaParticipation.aprobado
        ? "COMPLETED"
        : ("ATTENDING" as ParticipationStatus),
      ...
      } as ParticipationGrading,
      attendanceRecords: [], // Se puede implementar si es necesario
      minimumAttendancePercentage:
        prismaParticipation.inscripcion.evento
          ?.porcentaje_asistencia_aprobacion || 80,
      minimumGradeRequired: undefined, // Solo para cursos
      totalSessions: Math.ceil(
        (prismaParticipation.inscripcion.evento?.dur_eve || 8) / 2
      ), // Estimado
      sessionsAttended: Math.ceil(
        (prismaParticipation.asi_par / 100) *
