...
export class CertificateController extends BaseController {
   ...
   */
  public async getCompletedParticipations(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.usuario?.id_usu || req.uid;
      const prisma = this.container.getPrismaClient();
   ...
   */
  public async generateEventCertificate(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
   ...
   */
  public async generateCourseCertificate(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { idParticipacion } = req.params;
      const userId = req.usuario?.id_usu || req.uid;
      const prisma = this.container.getPrismaClient();
   ...
   */
  private async generateEventCertificatePDF(participacion: any): Promise<Buffer> {
   ...
   */
  private async generateCourseCertificatePDF(participacion: any): Promise<Buffer> {
    const PDFDocument = require('pdfkit');
