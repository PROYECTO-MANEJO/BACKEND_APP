...
export class DeveloperController {
   ...
   */
  public async getAvailableDevelopers(
    req: AuthenticatedRequest,
    res: Response
  ): Promise<void> {
    try {
      ...
      if (!userRole || !["ADMINISTRADOR", "MASTER"].includes(userRole)) {
        res.status(403).json({
          success: false,
          message:
            "Solo los administradores pueden ver desarrolladores disponibles",
        });
        return;
      }
   ...
   */
  public async getDeveloperStatistics(
    req: AuthenticatedRequest,
    res: Response
  ): Promise<void> {
    try {
   ...
   */
  public async getWorkloadStatistics(
    req: AuthenticatedRequest,
    res: Response
  ): Promise<void> {
    try {
      ...
      const repository = this.container.developerRepository as any;
   ...
   */
  public async getRecommendedDevelopers(
    req: AuthenticatedRequest,
    res: Response
  ): Promise<void> {
    try {
      ...
      const repository = this.container.developerRepository;
      const developers = await repository.findRecommendedForRequest(
        requestType,
        skills
      );
   ...
   */
  public async updateGithubCredentials(
    req: AuthenticatedRequest,
    res: Response
  ): Promise<void> {
    try {
      ...
      const updatedDeveloper = developer.updateGithubCredentials(
        githubToken,
        githubUsername
      );
