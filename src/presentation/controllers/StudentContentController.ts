...
export class StudentContentController extends BaseController {
   ...
   */
  public async getAvailableEvents(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
   ...
   */
  public async getAvailableCourses(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.usuario?.id_usu || req.uid;
      if (!userId) {
        res.status(401).json({
