...
export class UserManagementController extends BaseController {
      ...
      if (search) {
        where.OR = [
          ...
          { 
            cuentas: {
              some: {
                cor_cue: { contains: search as string, mode: 'insensitive' }
              }
            }
          }
        ];
   ...
   */
  public async getUserByCedula(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
  ...
  public async updateUser(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (nom_usu1) updateData.nom_usu1 = nom_usu1;
      if (nom_usu2 !== undefined) updateData.nom_usu2 = nom_usu2 || '';
      if (ape_usu1) updateData.ape_usu1 = ape_usu1;
      if (ape_usu2 !== undefined) updateData.ape_usu2 = ape_usu2 || '';
      if (fec_nac_usu) updateData.fec_nac_usu = new Date(fec_nac_usu);
        if (num_tel_usu !== undefined) updateData.num_tel_usu = num_tel_usu || null;
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const cuentaUpdateData: any = {};
        if (cor_cue) cuentaUpdateData.cor_cue = cor_cue;
  ...
  public async getUserStats(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const rolesStats = usuariosPorRol.reduce((acc: any, item) => {
