...
export class ChangeRequestManagementService {
  constructor(
    private changeRequestRepository: IChangeRequestRepository,
    private gitHubService: IGitHubIntegrationService,
   ...
   */
  async submitChangeRequest(changeRequestId: string): Promise<ChangeRequest> {
    ...
    return await this.changeRequestRepository.update(
      changeRequestId,
      changeRequest
    );
  }
  ...
  async assignReviewer(
    ...
    if (!changeRequest.isSubmitted()) {
      throw new Error(
        "Solo se pueden asignar revisores a solicitudes enviadas"
      );
    }
  ...
  async approveChangeRequest(
    ...
    targetDate?: Date,
    reviewNotes?: string,
    createGitHubIssue: boolean = true
  ): Promise<ChangeRequest> {
    const changeRequest = await this.getChangeRequestById(changeRequestId);
  ...
  async assignDeveloper(
    ...
    if (!changeRequest.canStartDevelopment()) {
      throw new Error(
        "La solicitud no puede iniciar desarrollo en su estado actual"
      );
    }

    changeRequest.startDevelopment(
      developerId,
      this.formatUserName(developer),
      technicalDetails
    );
    ...
    await this.notificationService.notifyDeveloperAssigned(
      changeRequestId,
      developerId
    );
  ...
  async moveToTesting(
    ...
    if (changeRequest.status !== "EN_DESARROLLO") {
      throw new Error(
        "Solo se pueden mover a pruebas solicitudes en desarrollo"
      );
    }
    ...
    if (createPullRequest && changeRequest.githubIntegration?.branchName) {
      try {
        const prResult = await this.gitHubService.createPullRequest(
          changeRequest,
          changeRequest.githubIntegration.branchName
        );
        changeRequest.updateGitHubIntegration({
          ...changeRequest.githubIntegration,
    ...
    if (changeRequest.githubIntegration?.issueNumber) {
      try {
        await this.gitHubService.closeIssue(
          changeRequest.githubIntegration.issueNumber,
          "Completed"
        );
      } catch (error) {
  ...
  async updateAcceptanceCriteria(
    ...
    if (!hasPermission) {
      throw new Error(
        "No tienes permisos para modificar los criterios de aceptación"
      );
    }
   ...
   */
  async syncWithGitHub(changeRequestId: string): Promise<ChangeRequest> {
    const changeRequest = await this.getChangeRequestById(changeRequestId);

    try {
      const integration = await this.gitHubService.syncChangeRequestWithGitHub(
        changeRequestId
      );
  ...
  async getDeveloperAssignments(
    ...
    const assignments = await this.changeRequestRepository.findByDeveloper(
      developerId
    );
   ...
   */
  async getPendingReviews(): Promise<ChangeRequest[]> {
   ...
   */
  async getOverdueRequests(): Promise<ChangeRequest[]> {
  ...
  async getChangeRequestStatistics(filters?: {
    ...
    const requests = await this.changeRequestRepository.findWithFilters(
      filters || {}
    );
    ...
    const stats = {
      ...
      byType: {} as Record<ChangeRequestType, number>,
   ...
   */
  private async getChangeRequestById(id: string): Promise<ChangeRequest> {
