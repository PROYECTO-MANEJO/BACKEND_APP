...
export class CourseManagementService {
  ...
  async createCourse(courseData: CourseData): Promise<Course> {
    ...
    const organizerExists = await this.userRepository.exists(
      courseData.ced_org_cur
    );
    if (!organizerExists) {
    ...
    const conflictingCourses =
      await this.courseRepository.findConflictingCourses(
        courseData.ced_org_cur,
        courseData.fec_ini_cur,
        courseData.fec_fin_cur
      );

    if (conflictingCourses.length > 0) {
      throw new Error(
        "El organizador ya tiene cursos programados en las fechas especificadas"
      );
    }

    // 4. Validar carreras si es para carrera específica
    if (
      courseData.tipo_audiencia_cur === "CARRERA_ESPECIFICA" &&
      courseData.carreras
    ) {
  ...
  async updateCourse(
    ...
    if (updateData.fec_ini_cur || updateData.fec_fin_cur) {
      ...
      if (conflictingCourses.length > 0) {
        throw new Error(
          "Las nuevas fechas conflictan con otros cursos del organizador"
        );
      }
    }
    ...
    if (updateData.capacidad_max_cur) {
      const enrolledCount = await this.courseRepository.getEnrolledCount(
        courseId
      );
      if (updateData.capacidad_max_cur < enrolledCount) {
        throw new Error(
          `No se puede reducir la capacidad por debajo del número actual de inscritos (${enrolledCount})`
        );
      }
    }
   ...
   * ✅ CERRAR CURSO
   */
  async closeCourse(courseId: string, organizerId: string): Promise<Course> {
    // 1. Obtener curso
  ...
  async updateCourseCarerIds(
    ...
    if (course.ced_org_cur !== organizerId) {
      throw new Error(
        "Solo el organizador puede actualizar las carreras del curso"
      );
    }
    ...
    if (!course.isForSpecificCareer()) {
      throw new Error(
        "Solo los cursos de carrera específica pueden tener carreras asignadas"
      );
    }
    ...
    return (await this.courseRepository.findById(courseId)) as Course;
  }

  /**
   * ✅ OBTENER CURSOS DISPONIBLES PARA USUARIO
   */
  async getAvailableCoursesForUser(userId?: string): Promise<Course[]> {
  ...
  async canUserEnroll(
    ...
    const isAlreadyEnrolled = await this.courseRepository.isUserEnrolled(
      courseId,
      userId
    );
    ...
    if (course.isForSpecificCareer()) {
      const courseCareerIds = await this.courseRepository.getCourseCareerIds(
        courseId
      );
   ...
   */
  private async validateCareerIds(careerIds: number[]): Promise<void> {
  ...
  async getCourseStatistics(courseId: string): Promise<{
    ...
    const canEnroll =
      availableSpots > 0 && course.isUpcoming() && course.isActive();
  ...
  async validateScheduleConflicts(
    ...
    return await this.courseRepository.findConflictingCourses(
      organizerId,
      startDate,
      endDate,
      excludeCourseId
